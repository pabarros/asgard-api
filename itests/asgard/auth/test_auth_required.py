...
class AuthRequiredTest(TestCase):
    async def setUp(self):
        ...
        with mock.patch.object(hollowman.conf, "HOLLOWMAN_DB_URL", TEST_PGSQL_DSN):
            reload(asgard.db)

        self.asyncworker_settings_patcher = mock.patch(
            "asyncworker.signals.handlers.http.settings", self.asyncworker_settings_mock
        )
    ...
    #        auth_header = {"Authorization": "JWT {}".format(jwt_token.decode("utf-8"))}
    ...
    #            "{}?jwt={}".format(conf.REDIRECT_AFTER_LOGIN, jwt.decode.return_value)
    #        )
    ...
    #        )
    #        self.assertEqual(
    #            self.user.accounts[0].name, token_content["current_account"]["name"]
    #        )
    #        self.assertEqual(
