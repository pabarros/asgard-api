...
def get_task_data(task_id):
    ...
    task_info = requests.get(
        f"{MESOS_ADDRESSES[0]}/tasks?task_id={task_id_with_namespace}"
    ).json()["tasks"]
    ...
    slave_info = requests.get(
        f"{MESOS_ADDRESSES[0]}/slaves?slave_id={slave_id}"
    ).json()["slaves"][0]
    slave_ip = slave_info["hostname"]

    slave_state = requests.get(f"http://{slave_ip}:5051/state").json()
    framework_info = [
        fwk for fwk in slave_state["frameworks"] if fwk["id"] == framework_id
    ][0]
    execuor_info = [
        executor
        for executor in framework_info["executors"]
        if executor["id"] == task_id_with_namespace
    ]
    if not execuor_info:
        execuor_info = [
            executor
            for executor in framework_info["completed_executors"]
            if executor["id"] == task_id_with_namespace
        ]
...
def task_files_list(task_id, user):
    ...
    files_info = requests.get(
        f"http://{slave_ip}:5051/files/browse?path={sandbox_directory}"
    ).json()
    for file_data in files_info:
        file_data["path"] = file_data["path"].replace(sandbox_directory, "", 1)

    return Response(
        response=json.dumps(files_info), status=200, mimetype="application/json"
    )
