...
PLUGINS_LOAD_STATUS = {
    "plugins": {},
    "stats": {"load_ok": 0, "load_failed": 0, "load_total": 0},
}


def register_plugin(plugin_id):
    plugin_data = {"id": plugin_id, "info": {"modules": ["ui"]}}
...
def load_all_metrics_plugins(
    flask_application, get_plugin_logger_instance=get_plugin_logger_instance
):
    ...
    for entrypoint in all_metric_plugins:
        try:
            ...
            flask_application.register_blueprint(
                plugin_data["blueprint"], url_prefix=url_prefix
            )
            logger.info(
                {
