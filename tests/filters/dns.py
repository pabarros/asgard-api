...
class DNSRequestFilterTest(TestCase):
  ...
  def test_add_dns_enty_when_app_has_no_docker_parameters(self):
    ...
    self.assertTrue('parameters' in modiified_request.get_json()['container']['docker'])

    docker_parameters = modiified_request.get_json()['container']['docker']['parameters']
  ...
  def test_remove_any_unwanted_dns_entry_multi_app(self):
    ...
    docker_parameters = modiified_request.get_json()[0]['container']['docker']['parameters']
    self.assertEqual(1, len(docker_parameters))
    self.assertEqual("dns", docker_parameters[0]['key']);
    self.assertEqual("172.17.0.1", docker_parameters[0]['value']);

    docker_parameters = modiified_request.get_json()[1]['container']['docker']['parameters']
    self.assertEqual(1, len(docker_parameters))
    self.assertEqual("dns", docker_parameters[0]['key']);
    self.assertEqual("172.17.0.1", docker_parameters[0]['value']);
  ...
  def test_add_dns_entry_when_creating_1_depth_apps_via_groups_endpoint(self):
    ...
    docker_parameters = modiified_request.get_json()['apps'][0]['container']['docker']['parameters']
    self.assertEqual(1, len(docker_parameters))
    self.assertEqual("dns", docker_parameters[0]['key']);
    self.assertEqual("172.17.0.1", docker_parameters[0]['value']);
    
    docker_parameters = modiified_request.get_json()['apps'][1]['container']['docker']['parameters']
    self.assertEqual(1, len(docker_parameters))
    self.assertEqual("dns", docker_parameters[0]['key']);
    self.assertEqual("172.17.0.1", docker_parameters[0]['value']);
  ...
  def test_add_dns_entry_on_creating_2_depth_groups_with_apps_via_groups_endpoint(self):
    ...
    docker_parameters = modiified_request.get_json()['groups'][0]['apps'][0]['container']['docker']['parameters']
    self.assertEqual(1, len(docker_parameters))
    self.assertEqual("dns", docker_parameters[0]['key']);
    self.assertEqual("172.17.0.1", docker_parameters[0]['value']);

    docker_parameters = modiified_request.get_json()['groups'][0]['groups'][0]['apps'][0]['container']['docker']['parameters']
    self.assertEqual(1, len(docker_parameters))
    self.assertEqual("dns", docker_parameters[0]['key']);
    self.assertEqual("172.17.0.1", docker_parameters[0]['value']);
