...
class PluginLoaderTest(unittest.TestCase):
    ...
    def test_plugin_load_plugins_populate_internal_registry(self):
        ...
        with mock.patch.multiple(conf, LOGLEVEL="DEBUG"):
            ...
            plugins.load_all_metrics_plugins(application, get_plugin_logger_instance=lambda plugin_id: logger_mock)
            self.assertEqual(4, len(plugins.PLUGINS_LOAD_STATUS['plugins']['asgard-api-plugin-metrics-example-1']))
            plugin_status_data = plugins.PLUGINS_LOAD_STATUS['plugins']['asgard-api-plugin-metrics-example-1']
