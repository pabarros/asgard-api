...
class HTTPWrapperTest(TestCase):
    def test_it_recognizes_group_requests(self):
        ...
        with application.test_request_context(
            "/v2/groups/xablau/Xena", method="PUT", data=b""
        ) as ctx:
            request_parser = Request(ctx.request)
            self.assertTrue(request_parser.is_group_request())
            self.assertFalse(request_parser.is_app_request())
        ...
        with application.test_request_context(
            "/v2/groups/xablau", method="DELETE", data=b""
        ) as ctx:
            request_parser = Request(ctx.request)
            self.assertTrue(request_parser.is_group_request())
            self.assertFalse(request_parser.is_app_request())
    ...
    def test_is_delete(self):
        with application.test_request_context(
            "/v2/apps/app0", method="DELETE", data=b""
        ) as ctx:
            request_parser = Request(ctx.request)
    ...
    def test_return_correct_request_resource(self):
        ...
        for (
            ...
            self.assertEqual(
                request_wrapper.request_resource, expected_request_resource
            )
            self.assertEqual(
                response_wrapper.request_resource, expected_request_resource
            )
