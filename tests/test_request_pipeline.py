...
class RequestPipelineTest(unittest.TestCase):
    ...
    def test_do_not_call_task_method_if_filter_does_not_implement(self):
        ...
        pipeline = {OperationType.WRITE: [DummyFilter()]}

        request_data = {"ids": ["task_0"]}
        request_app = MarathonTask.from_json({"id": request_data["ids"][0]})
        original_app = (
            request_app
        )  # Por enquanto não temos como passar a original task
        ...
        with application.test_request_context(
            ...
            with patch.object(
                request, "split", return_value=[(request_app, original_app)]
            ):
                filtered_request = dispatch(
                    self.user, request, filters_pipeline=pipeline
                )
    ...
    def test_call_write_task_if_is_v2_tasks_request(self):
        ...
        pipeline = {OperationType.WRITE: [DummyFilter(), DummyFilter2()]}
    ...
    def test_update_app_remove_all_constraints(self):
        ...
        original_app = AsgardApp.from_json(
    ...
    def test_dispatch_should_pass_an_instance_if_SieveMarathonApp_to_filters(
        ...
        pipeline = {OperationType.WRITE: [AddNewConstraintFilter()]}
    ...
    def test_preserve_constraints_added_by_filter(self):
        ...
        with application.test_request_context(
            ...
            filtered_request_app = AsgardApp.from_json(
    ...
    def _check_other_fields(self, skip_field_names, filtered_app):
        json_repr = json.loads(
            json.dumps(filtered_app, cls=HTTPWrapper.json_encoder)
        )

        for key in REMOVABLE_KEYS:
            if key not in skip_field_names:
                self.assertEqual(
                    self.single_full_app_fixture[key],
                    json_repr[key],
                    "key `{}` foi alterada".format(key),
                )
    ...
    def test_preserve_labels_added_by_filter(self):
        ...
        pipeline = {OperationType.WRITE: [AddNewLabelFilter()]}
        request_data = {"labels": {}}
    ...
    def test_preserve_upgrade_strategy_added_by_filter(self):
        ...
        pipeline = {OperationType.WRITE: [AddNewUpgradeStrategyFilter()]}
    ...
    def test_preserve_envs_added_by_filter(self):
        ...
        pipeline = {OperationType.WRITE: [AddNewEnvFilter()]}
        request_data = {"env": []}
    ...
    def test_preserve_healthchecks_added_by_filter(self):
        ...
        pipeline = {OperationType.WRITE: [AddNewHealthCheckFilter()]}
